...
- For each month M ≥ effective_from (and ≥ member's join month), the posting engine creates one approved `allowance` credit for the amount in force during M. Amount 0 = paused.
...
3. All inserts rely on the unique partial index → naturally idempotent; run inside one transaction per group.
...
- `009_ledger_v2`: entry_type, direction, nullable chore_id, loan_id, period, note, backfill, unique partial index.
- `010_allowances`.
...
10. **Cloud deploy + HTTPS** (Fly.io/VPS), backups, then app-store distribution.
...
|---|---|---|
| **0.1** FE deps alignment | Align `app/package.json` to one Expo SDK (latest stable); app boots on web + Android | `npx expo-doctor` clean; login→dashboard works on web & Android |
| **0.2** Generated API types + Query | Add `openapi-typescript` codegen script; introduce TanStack Query provider; migrate existing screens' fetches to hooks | codegen script in `app/package.json`; no hand-written response types remain in screens |
| **0.3** Auth redirects (bug) | 401 anywhere and logout both land on login screen, on all 3 platforms | manual matrix: expired token, logout — web + Android |
| **0.4** ∥ Fix chores tab (bug) | Chores list/create/edit/soft-delete per §5.5; system chore pinned, not editable | head CRUD works; member read-only; system chore protected (API already enforces) |

### Phase 1 — Ledger v2 (backend first)
| WP | Scope | Acceptance criteria |
|---|---|---|
| **1.1** Money to minor units | Migration 008; Go models/handlers to int64; openapi.yaml amounts → integer; regenerate FE types; FE formats via `AmountText` | all tests pass; grep shows no float64 money in backend |
| **1.2** Ledger v2 schema + API | Migrations 009 + 012; entry_type/direction/note; settlement rows migrated; `/settlements` + `/pending` removed; ledger create/list per §6.1; balance = credits−debits; openapi updated | integration tests: balance math, member-can't-see-others, head-only settlement/adjustment |
| **1.3** FE unified ledger | Rebuild group Overview per §7.1/§7.2 for both roles; inline approve/reject; strikethrough rejected; month grouping; remove Pending/Settlements screens | matches `docs/fe-flow.md` + `docs/FE-bugs.md` behaviors on web + Android |
...
### Phase 4 — UX polish & release readiness
| WP | Scope | Acceptance criteria |
|---|---|---|
| **4.1** Design-system pass | Implement §7.4 tokens/components; sweep all screens | no inline hex colors in screens; visual consistency checklist |
...
3. **Verify**: BE → `make -C backend test lint`; FE → `npx tsc --noEmit` + boot on web (`npm run web`) and exercise the changed flow. State in the PR what you ran.
